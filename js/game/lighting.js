...
function createFlashlight(scene, player) {
  ...
  const flashlight = new THREE.SpotLight(
    ...
    4.0, // Intensity
    250, // Range
    Math.PI / 6, // Wider spotlight angle (30 degrees)
    0.8, // Higher penumbra for softer, more natural edge
    2.5 // Higher decay for more natural falloff
  );
  ...
  // Position flashlight slightly above the player
...
// Add visual cone for flashlight beam with more triangular shape
function addFlashlightCone(flashlight) {
  // Create cone geometry for flashlight beam
  // Using a triangular-shaped cone for more realistic flashlight effect
  const coneLength = 300;
  const coneRadius = Math.tan(flashlight.angle) * coneLength;
  
  // Create custom geometry for a more triangular beam
  const coneGeometry = new THREE.CylinderGeometry(coneRadius, 0.1, coneLength, 4, 1, true);
  
  // Rotate the geometry to make it more diamond-shaped
  ...
  // Add a second inner cone for a more intense center beam
  const innerConeGeometry = new THREE.CylinderGeometry(coneRadius * 0.6, 0.1, coneLength * 1.1, 4, 1, true);
  innerConeGeometry.rotateY(Math.PI / 4);
  ...
  const innerLightCone = new THREE.Mesh(innerConeGeometry, innerConeMaterial);
  innerLightCone.rotation.x = Math.PI;
  innerLightCone.position.set(0, 0, -coneLength/2);
  
  // Store reference in userData
  flashlight.userData.cone = lightCone;
  flashlight.userData.innerCone = innerLightCone;
  
  // Add cones to flashlight
  flashlight.add(lightCone);
  flashlight.add(innerLightCone);
  
  // Add a small point light at the source to simulate flashlight bulb glow
  const bulbLight = new THREE.PointLight(0xffffaa, 0.5, 10);
  bulbLight.position.set(0, 0, 0);
  flashlight.add(bulbLight);
}
...
function updateFlashlight(flashlight, player, deltaTime) {
  ...
  // Update inner cone as well
  if (batteryData.innerCone) {
      batteryData.innerCone.scale.set(batteryFactor, batteryFactor, batteryFactor);
  }
  
  // If battery is critically low, add flickering effect
  if (batteryData.battery < 15) {
      const flickerAmount = (Math.random() - 0.5) * 0.4;
      flashlight.intensity = batteryData.originalIntensity * (0.7 + flickerAmount);
      
      // Make the cone flicker too for visual effect
      if (batteryData.cone) {
          const visualFlicker = 0.85 + (Math.random() * 0.3);
          batteryData.cone.scale.set(
              batteryFactor * visualFlicker,
              batteryFactor * visualFlicker,
              batteryFactor * visualFlicker
          );
          
          if (batteryData.innerCone) {
              batteryData.innerCone.scale.set(
                  batteryFactor * visualFlicker,
                  batteryFactor * visualFlicker,
                  batteryFactor * visualFlicker
              );
